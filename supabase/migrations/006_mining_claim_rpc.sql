...
begin
  ...
  -- قفل صف المستخدم لمنع Claim مزدوج
  select
  ...
  -- الأصدقاء الذين قاموا Claim خلال آخر 24 ساعة
  select count(*)
  into v_active_friends
  from public.users as f
  where f.referred_by = p_telegram_id
    and f.last_claim >=
      v_now - interval '24 hours';

  -- +5% من السرعة الأساسية لكل صديق نشط
  v_total_rate :=
    v_base_rate +
    (
      v_active_friends *
  ...
  v_claimed :=
    v_elapsed_seconds * v_total_rate;
