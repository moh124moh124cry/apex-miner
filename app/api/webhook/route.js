...
const rawAllowedLinkPrefixes = (process.env.ALLOWED_LINK_PREFIXES || '')
  .split(',')
...
const blockOtherBots = process.env.BLOCK_OTHER_BOTS === 'true';
...
const MAX_MODERATION_STATE_ENTRIES = 10_000;

// ======================================================
// Lightweight per-instance moderation state
// ======================================================
...
function setBoundedMapEntry(map, key, value) {
  ...
  while (map.size > MAX_MODERATION_STATE_ENTRIES) {
...
function normalizeAllowedDomain(rawDomain) {
...
function createAllowedPrefixRule(rawPrefix) {
  const value = String(rawPrefix || '').trim();
  if (!value) return null;

  if (value.toLowerCase().startsWith('tg://')) {
    return {
      type: 'telegram-deep-link',
...
const allowedLinkPrefixRules = rawAllowedLinkPrefixes
  .map(createAllowedPrefixRule)
  .filter(Boolean);

function pathMatchesPrefix(candidatePath, allowedPath) {
  const candidate = (candidatePath || '/').toLowerCase();
  const prefix = (allowedPath || '/').toLowerCase();
...
function telegramDeepLinkMatches(rawUrl, ruleValue) {
  const candidate = String(rawUrl || '').trim().toLowerCase();

  if (candidate === ruleValue) return true;
  if (!candidate.startsWith(ruleValue)) return false;

  const nextCharacter = candidate[ruleValue.length];
  return ['/', '?', '#', '&'].includes(nextCharacter);
}

function matchesAllowedPrefix(rawUrl, rule) {
  if (rule.type === 'telegram-deep-link') {
    return telegramDeepLinkMatches(rawUrl, rule.value);
  }
  ...
  if (parsed.protocol.toLowerCase() !== rule.protocol) return false;
  if (hostname !== rule.hostname) return false;
  if ((parsed.port || '') !== rule.port) return false;

  return pathMatchesPrefix(parsed.pathname, rule.pathname);
}

// ======================================================
// Extract links from all supported Telegram surfaces
// ======================================================
  ...
  for (const row of inlineKeyboard) {
    ...
    for (const button of row) {
      ...
      if (button.login_url?.url) {
        found.add(button.login_url.url);
      }

      if (button.web_app?.url) {
        found.add(button.web_app.url);
      }
    }
  }
...
function isAllowedUrl(rawUrl) {
  if (
    allowedLinkPrefixRules.some((rule) =>
      matchesAllowedPrefix(rawUrl, rule)
    )
  ) {
    return true;
  }
...
function containsUnauthorizedLink(message) {
  ...
  if (
    allowedLinkPrefixRules.length === 0 &&
...
const spamKeywords = [...defaultSpamKeywords, ...extraSpamKeywords];
...
function isFlooding(ctx) {
  ...
  current.timestamps = current.timestamps.filter(
    (item) => timestamp - item <= FLOOD_WINDOW_MS
  );

  current.timestamps.push(timestamp);
  current.updatedAt = timestamp;

  setBoundedMapEntry(floodState, key, current);

  return current.timestamps.length > FLOOD_MAX_MESSAGES;
}

function isRepeatedMessage(ctx) {
  const text = normalizeText(getMessageText(ctx.message));
  ...
  if (
    current.text !== text ||
    timestamp - current.firstAt > REPEAT_WINDOW_MS
  ) {
    setBoundedMapEntry(repeatState, key, {
      text,
      count: 1,
      firstAt: timestamp,
      updatedAt: timestamp,
  ...
  setBoundedMapEntry(repeatState, key, current);

  return current.count >= REPEAT_MAX_SAME_MESSAGE;
}
...
async function isAdmin(ctx) {
  try {
    ...
    setBoundedMapEntry(adminCache, key, {
      isAdmin: result,
    ...
    // Fail open for moderation actions to avoid punishing
    // a real administrator during a Telegram API outage.
    return true;
  }
}
...
async function muteUser(ctx, seconds = MUTE_SECONDS) {
  try {
    ...
    await ctx.telegram.callApi('restrictChatMember', {
      chat_id: ctx.chat.id,
      user_id: ctx.from.id,
      permissions: {
...
function recordViolation(ctx) {
  ...
  current.timestamps = current.timestamps.filter(
    (item) => timestamp - item <= VIOLATION_WINDOW_MS
  );

  current.timestamps.push(timestamp);
  current.updatedAt = timestamp;

  setBoundedMapEntry(violationState, key, current);

  return current.timestamps.length;
}

async function moderateViolation(ctx, reason, forceMute = false) {
  ...
  const violations = recordViolation(ctx);

  if (forceMute || violations >= VIOLATIONS_BEFORE_MUTE) {
    await muteUser(ctx);
  }
...
bot.command('start', async (ctx) => {
  try {
    ...
    if (!appUrl) {
      await ctx.reply('Welcome to ApexMiner! 🚀');
      return;
    }
...
bot.on('new_chat_members', async (ctx) => {
  try {
    ...
    const humanMembers = members.filter((member) => !member.is_bot);
...
bot.on('message', async (ctx) => {
  try {
    ...
    if (blockOtherBots && ctx.from.is_bot) {
      await moderateViolation(ctx, 'OTHER_BOT', true);
      return;
    }

    // Unauthorized links: DELETE ONLY. No mute, no ban,
    // and no violation counter.
    if (containsUnauthorizedLink(message)) {
      if (!(await isAdmin(ctx))) {
        await safeDelete(ctx);
    ...
    if (looksLikeSpam(message)) {
      await moderateViolation(ctx, 'SPAM_CONTENT');
      return;
    }

    if (isRepeatedMessage(ctx)) {
      await moderateViolation(ctx, 'REPEATED_MESSAGE', true);
      return;
    }

    if (isFlooding(ctx)) {
      await moderateViolation(ctx, 'FLOOD', true);
    }
  } catch (error) {
    console.error('Moderation error:', error);
  } finally {
    if (Math.random() < 0.02) {
      cleanupMap(floodState, FLOOD_WINDOW_MS * 3);
      cleanupMap(repeatState, REPEAT_WINDOW_MS * 3);
      cleanupMap(violationState, VIOLATION_WINDOW_MS * 2);
...
export async function POST(req) {
  try {
    ...
    // Fail closed: if the server-side secret disappears,
    // the webhook never becomes publicly writable.
    if (!webhookSecret) {
      console.error('Webhook security configuration missing');
    ...
    if (!safeEqualSecret(receivedSecret, webhookSecret)) {
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      );
